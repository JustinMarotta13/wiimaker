...
use args::{Cli, Cmd};
use cmds::{asset, editor, entity, input, project, scene, sorting_layer, tilemap};
use util::find_root;
...
fn main() -> Result<()> {
    ...
    match cli.cmd {
        ...
        Cmd::SortingLayer { cmd } => sorting_layer::sorting_layer_cmd(&root, cmd, cli.json),
        Cmd::Input { cmd } => input::input_cmd(cmd, cli.json),
    }
}
