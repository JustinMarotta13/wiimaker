...
- Tilemap: viewport **Paint / Erase / Pick** (right-click erases while Painting). Inspector **+ Tilemap** (32×18, cell 16) · grid w/h · cell · origin · palette (id/color/sprite/**Anim**/Override FPS/**Auto Tile** Off·Same id·Solid/Variants) · **Brush**. Palette id DragValue is `1..=u16::MAX` (CLI `--id` is `u16`; do not clamp to 32). Painted NESW badge next to Brush when auto-tile is on. Selected tilemap AABB outlined; paint stamps `tile_brush_id` + solid onto the targeted grid. Hover status shows `mask`. Scene **Edit** ticks palette anims (`World::tick_tilemaps`) so water/lava preview without Play.
