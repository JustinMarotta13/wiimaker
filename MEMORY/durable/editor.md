...
- Viewport blit contain-fits 640×480 into the leftover Scene well (upscale allowed; no `.min(1.0)` scale cap). Map picks with `pointer_to_scene` using the **image rect**, not the full CentralPanel. Hierarchy/Inspector/Project sizes are **app-owned** (`hierarchy_width` 180..=520 / `inspector_width` 220..=520 / `project_panel_height` 140..=560 + `exact_width`/`exact_height` + 4px edge drag, `resizable(false)`). Never `set_min_width(available_width())` / `set_max_width(available_width())` on those panels — a long tree expands PanelState and steals the Scene well. New ids (`hierarchy_dock`, `inspector_dock`) discard stale PanelState. Virtualize Hierarchy rows when a parent has hundreds of children. Scene/Game well grows when a splitter moves.
...
- SidePanel width flicker: do **not** fight PanelState with `set_min_width(available_width())`. Own the width (`hierarchy_width` / `inspector_width`) and `exact_width` + `resizable(false)` + edge drag (Hierarchy **right** edge, Inspector **left** edge, width += ±drag_delta.x). Truncate long Inspector labels so they cannot expand the panel.
...
- Project panel height is **app-owned** (`project_panel_height` + `exact_height`, clamp ~140..=560). Do not let egui derive bottom-panel height from content. New panel id if old `PanelState` is corrupted. Manual top-edge drag to resize; shrinking it gives Scene more room.
