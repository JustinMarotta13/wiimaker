...
- Until Rust `staticlib` lands, Dolphin play uses the C scene player + GX textured quads (sprites + tilemap cells) / untextured discs, text, and color tiles; host keeps `wiimaker-scene` JSON hydrate + `SpriteCatalog`. WSCN/stub_game still skip cameras.
...
- Wii ASND audio oneshots (host `AudioSource` already plays WAV; C comments only).
