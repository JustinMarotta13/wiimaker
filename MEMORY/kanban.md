...
| Inspector | Transform + Sprite/Disc/Camera/Tilemap, enable checkbox, catalog combo, tile palette |
...
Runtime already: `World` (named entities, Transform, Sprite, Disc, Camera marker, Tilemap, `tag: u32`), `DrawList` IR, GCN-layout `Input` (WASD/arrows → stick + D-pad), 60 Hz `Clock`, `render_world` sorts by component `z` (tile cells as sprites/colored quads), parented local transforms, sprite UV/pivot, `.wpack` cook, WSCN0003 bake (UV + pivot + length-prefixed Tilemap), `wiimaker build` / `dolphin` / `play-wii`. Queries: `tile_solid` / `world_to_cell` / `tile_solid_world`.

**Not present:** colliders, triggers, sprite clips, audio playback, camera used at render time, named sorting layers, prefab variants, runtime scene API, text/UI, play-mode running the game crate, Wii GX draw of tilemaps (payload skipped).

---

## Now

### 1. AABB collider + overlap query — **GUI + CLI**
Unity: BoxCollider2D. Unblocks walls-as-entities *and* dots until tilemap is enough.

- Component `Collider { kind: Aabb | Circle, size/radius, solid: bool }`.
- `overlaps(world, a, b)` / `move_and_collide(id, delta)`.
- Hydrate + bake (WSCN bump if Wii needs it; host-first is enough for the morning).
- **GUI:** Inspector fields + viewport outline gizmo.
- **CLI:** `entity add-component <game> --name X Collider --w --h` · `entity overlaps`.
- **Test:** Player disc vs Wall_*; unit test two AABBs; pac-man walls stop the walker without the ASCII duplicate.
...
| `entity add-component` · `entity remove-component` · `entity set-component-enabled` | kinds: `Sprite` \| `Disc` \| `Tilemap` (`--cols --rows --cell`) |
...
Inspector: Edit Sprites… · Save as Prefab… · component enable checkbox · Remove · Tilemap grid/palette/Brush
...
**Ship AABB collider + overlap (Now #1).** Tilemap + solid cells are done — paint a maze, query `tile_solid`. Colliders still unlock entity walls / dots until triggers exist. Test: Player disc vs Wall_*; pac-man can drop the ASCII duplicate for entity-shaped solids.
